...
import { IAlarms } from 'src/interfaces/overview';
        ...
        if (item.type != '0x00') {
          if (index > 4) {
            ...
            _contentOther = _contentOther.concat(
              `${item.message || item.key} : ${item.count} \n`
            );
          } else {
            _values.push(item.count);
            _labels.push(item.message || item.key);
            _total += item.count;
          }
        }
      });
                ...
                <Row justify={'end'} gutter={16} className={styles.wrapper_btn}>
                  <Col>
                    <Button
                      ...
                      {
                        isOpenCollapse ? t(i18nKey.dashboard.expand) : t(i18nKey.dashboard.collapse)
                      }
                      <img
                        style={{
                          transition: 'all .2s ease',

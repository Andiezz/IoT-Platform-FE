      ...
      accountManagementListStore
        ...
        .then((rs) => {
          if (rs.responseCode === HTTP_STATUS_RESPONSE_KEY.SUCCESS) {
            form.setFieldValue('firstName', rs?.data?.first_name);
            form.setFieldValue('lastName', rs?.data?.last_name);
            form.setFieldValue('status', !!rs?.data?.is_active);
            form.setFieldValue('phoneCode', rs?.data?.phone_code);
            form.setFieldValue('phoneNumber', rs?.data?.phone_number);
            form.setFieldValue('email', rs?.data?.email);
            form.setFieldValue('role', rs?.data?.role._id);
          }
        });
    }
  }, [params.id]);
      ...
      <Spin spinning={loading}>
        ...
        <WidgetHeader>
          <Row
            ...
            <Col>
              <Typography.Title className={styles.wrapperForm_header_title} level={2}>
                {
                  isUpdateAccount
            ...
            <Col className={styles.wrapperForm_header_btn} sm={0} xs={0} md={12} lg={12} xl={12} xxl={12}>
              <Space>
                <Button className={styles.cancel} onClick={openConfirmCancel}>
                  {t(i18nKey.button.cancel)}
                </Button>
                {isUpdateAccount ? (
                  <Button className={styles.primary} onClick={onClickForm} type="primary">
                      ...
                      <Form.Item
                        ...
                        <Select
                          options={accountManagementListStore.listRole.map(
                            (item) => ({ value: item._id, label: `${t(i18nKey.permissionEntity.roleKeyToText[item.role])}`})
                          )}
                        />
                      </Form.Item>
                    </Col>
                    ...
                    <Row>
                      <Col span={24}>
                        <Form.Item
                          ...
                          <Radio.Group>
                            <Radio value={true}>{t(i18nKey.status.active)}</Radio>
                            <Radio value={false}>

...
import {
  ForgotPasswordDTO,
  IForgotPasswordBody
} from 'src/dto/forgot-password.dto';
...
const ForgotPassword: React.FC = () => {
  ...
  const onFormFinish = async (values: IForgotPasswordBody) => {
    ...
    await authService.forgotPassword(forgotDTO).then((rs: boolean | string) => {
      ...
      if (rs === true) {
        message.success(
          `${t(i18nKey.validation.emailOrPassword.sendMailResultSuccess)}`
        );
        setSendMail(true);
      }
      if (`${rs}` === messageResponse.inactiveAccountNotification) {
        message.error(
          `${t(i18nKey.messageIndicator.resetPasswordUnavailable)}`
        );
      }
    });
  };
  ...
  const onFinishFailed = () => {
    message.error(
      `${t(i18nKey.validation.emailOrPassword.sendMailResultFail)}`
    );
  };
        ...
        <Form
            ...
            <Col span={24}>
              <div className={styles.resetPassword_title}>
                <Typography.Title level={1}>
                  {t(i18nKey.loginPage.resetPasswordTitle)}
                </Typography.Title>
              </div>
            </Col>
          </Row>
          ...
          <Row>
            <Col span={24}>
              ...
              <Link
                to={PAGE_ROUTE.LOGIN}
                className={styles.resetPassword_btn_back}>
                <Button>
        ...
        <div style={{ width: '100%' }}>
          <Row style={{ textAlign: 'center' }}>
            ...
            <Col span={24}>
              <div className={styles.resetPassword_title}>
                <Typography.Title level={1}>
                  {t(i18nKey.loginPage.label.checkYourEmail)}
                </Typography.Title>
              </div>
            </Col>
            <Col span={24}>
            ...
            <Col span={24}>
              <Link to={PAGE_ROUTE.LOGIN}>
                <Button className={styles.resetPassword_btn_backBoder} block>
